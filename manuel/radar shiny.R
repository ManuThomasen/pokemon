...
make_radar <- function(df_one, stat_cols, title = NULL) {
  stopifnot(nrow(df_one) == 1)
  
  # Normalization constants
  norm_max <- getOption("radar_norm_max")
  if (is.null(norm_max) || !is.numeric(norm_max)) stop("Normalization maxima not set as numeric.")
  ...
  hovertext <- with(
    long_closed,
    paste0(
      "<b>", df_one$pokemon[1], "</b><br>",
      stat, ": ", value_raw,
      " (", scales::percent(value, accuracy = 1), " of max)"
    )
  )
  
  # Plotly-native radar avoids ggplotly polar warnings
  plotly::plot_ly(
    type = "scatterpolar",
    mode = "lines+markers",
    hoverinfo = "text"
...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    column(
      ...
      fluidRow(
        column(
          width = 6, class = "no-pad",
          div(class = "overflow-visible",
              plotlyOutput("radarA", height = "75%", width = "100%"))
        ),
        column(
          width = 6, class = "no-pad",
          div(class = "overflow-visible",
              plotlyOutput("radarB", height = "75%", width = "100%"))
        )
      )
    ),
...
server <- function(input, output, session) {
  ...
  # ---- Radar plots -----------------------------------------------------
  output$radarA <- renderPlotly({
    df <- oneA(); req(nrow(df) == 1)
    p <- make_radar(df, stat_cols, title = df$pokemon)
    col_hex <- if (!is.na(df$color_1)) df$color_1 else "#1f77b4"
    p %>%
      plotly::style(
  ...
  output$radarB <- renderPlotly({
    df <- oneB(); req(nrow(df) == 1)
    p <- make_radar(df, stat_cols, title = df$pokemon)
    col_hex <- if (!is.na(df$color_1)) df$color_1 else "#ff7f0e"
    p %>%
      plotly::style(
        line = list(color = col_hex),
        fillcolor = plotly::toRGB(col_hex, alpha = 0.35),
